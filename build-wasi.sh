...
then
    ...
    wasm-opt $PYTHON_WASM_FILE -o ${PYTHON_WASM_FILE%.*}_async.wasm --enable-multimemory --pass-arg=asyncify-memory@asyncify_unwind_stack_memory_heap --asyncify -O$OPTIMIZE_LEVEL
fi
